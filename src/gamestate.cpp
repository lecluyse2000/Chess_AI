...
[[nodiscard]] 
std::vector<std::pair<std::uint64_t, std::uint64_t> > Gamestate::get_white_rook_moves() const noexcept
{
    std::vector<std::pair<std::uint64_t, std::uint64_t> > moves;
    std::uint64_t current_bitboard = white_rooks;

    while (current_bitboard) {
        const std::uint64_t current_position = Util::get_least_sig_bit(current_bitboard);
        moves.emplace_back(std::make_pair(current_position, 
                                          generate_white_rook_moves(current_position, get_white_pieces(), get_black_pieces())));
...
[[nodiscard]] 
std::vector<std::pair<std::uint64_t, std::uint64_t> > Gamestate::get_black_rook_moves() const noexcept
{
    std::vector<std::pair<std::uint64_t, std::uint64_t> > moves;
    std::uint64_t current_bitboard = black_rooks;

    while (current_bitboard) {
        const std::uint64_t current_position = Util::get_least_sig_bit(current_bitboard);
        moves.emplace_back(std::make_pair(current_position, 
                                          generate_black_rook_moves(current_position, get_white_pieces(), get_black_pieces())));
